...
function CronJobCreatePage() {
  ...
  const formik = useFormik<CronJobInterface>({
    initialValues: {
      task_name: '',
      task_frequency: '',
      timing: new Date(new Date().toDateString()),
      parameters: '',
    ...
    >
      <Box rounded="md">
        ...
        <FormWrapper onSubmit={formik.handleSubmit}>
          ...
          <TextInput
            error={formik.errors.task_frequency}
            label={'Task Frequency'}
            props={{
              name: 'task_frequency',
              placeholder: 'Task Frequency',
              value: formik.values?.task_frequency,
              onChange: formik.handleChange,
            }}
          />

          <FormControl id="timing" mb="4">
            <FormLabel fontSize="1rem" fontWeight={600}>
              Timing
            </FormLabel>
            <DatePicker
              selected={formik.values?.timing ? new Date(formik.values?.timing) : null}
              onChange={(value: Date) => formik.setFieldValue('timing', value)}
            />
          </FormControl>
